#!/usr/bin/env python3
"""
Smart Cargo fleet simulator (stable GPS route mode).
...
Usage examples:
  python scripts/simulate_truck_jaffna_matara.py
  python scripts/simulate_truck_jaffna_matara.py --truck-id TRUCK02 --container-id CONT02
  python scripts/simulate_truck_jaffna_matara.py --route-steps 300 --interval 1.0
  python scripts/simulate_truck_jaffna_matara.py --loop-route
"""
...
def make_parser(defaults: Dict[str, str]) -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Publish simulated truck telemetry to EMQX (stable GPS route mode).",
...
def compute_route_position(args: argparse.Namespace, device: DeviceState) -> Tuple[float, float]:
...
def build_payload(args: argparse.Namespace, device: DeviceState, cycle_no: int) -> dict:
    ...
    lat, lon = compute_route_position(args, device)
    gps_fix = True
